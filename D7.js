...
console.log(concatStr("Ciao", "Belli"));
...
let arrayNumber = [1, 7, 3, 8, 5, 9, 12, 2, 11, 10];
...
function genNewArray(array, n) {
  const mappedNum = arrayNumber.map((num) => num + n);

  return mappedNum;
}
console.log(genNewArray(arrayNumber, 3));

/* ESERCIZIO 7 (map)
Scrivi una funzione che, dato un array di stringhe, ritorni un nuovo array contenente le lunghezze delle rispettive stringhe dell'array di partenza
es.: ["EPICODE", "is", "great"] => [7, 2, 5]
*/
arrayStr = ["Ciaone", "sono", "Sergio"];

function stringToNum(array) {
  return arrayStr.map((str) => str.length);
}

console.log(stringToNum(arrayStr));
...
console.log(shotsNum(arrayNumber));
...
function titleFilms(movies) {
  return movies.map((film) => film.Title);
}

console.log(titleFilms(movies));

/* ESERCIZIO 12 (filter)
  Scrivi una funzione per ottenere dall'array fornito solamente i film usciti nel millennio corrente.
*/

function currentFilms(movies) {
  return movies.filter((film) => parseInt(film.Year) >= 2000);
}

console.log(currentFilms(movies));

/* ESERCIZIO 13 (reduce)
  Scrivi una funzione per calcolare la somma di tutti gli anni in cui sono stati prodotti i film contenuti nell'array fornito.
*/

function sumYearFilms(movies) {
  const initialValue = 0;
  const sumTotal = movies.reduce((sum, film) => sum + parseInt(film.Year), initialValue);

  return sumTotal;
}

console.log(sumYearFilms(movies));
/* ESERCIZIO 14 (find)
...
const getFromID = function (imdbID) {
  const filmFromID = movies.filter((film) => film.imdbID === imdbID); //possiamo usare anche il find perche il find ne trova uno solo
  return filmFromID;
};
getFromID("tt4154796");
